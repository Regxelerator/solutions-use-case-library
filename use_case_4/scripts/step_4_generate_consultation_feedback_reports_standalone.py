...
def consolidating_the_results(
    ...
    consult_quest_summary_individual_json: str,
    consult_paper_info_json: str,
    executive_summary_json: str,
...
def merge_consecutive_cells(
    ...
    def merge_run_if_needed(
        ...
        """
        Merges a range of consecutive cells in a column if needed,
        otherwise writes a single value.
        ...
        """
        if rend > rstart:
            _worksheet.merge_range(
                rstart, col_index, rend, col_index, value, cell_format
            )
        else:
...
def main_excel(
    ...
    """
    summaries = load_json(consult_quest_summary_individual_json)
    ...
    for entry in summaries:
        ...
        ch_id_val = question_to_chapter_map.get(question_id)
        chapter_title = (
            chapter_map.get(str(ch_id_val), "Unknown Chapter")
