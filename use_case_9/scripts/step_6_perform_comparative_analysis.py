...
def comparative_analysis(output_root_dir: str) -> None:
    """
    For each report index, reads the per-year 'analysis_annual_report_<idx>.json' files
    from each year's interim_outputs folder, calls perform_comparative_analysis(...)
    once with the combined JSON, and writes the result to
    output_root_dir/comparative_analysis/comparative_analysis_annual_report_<idx>.json
    """
    year_dirs = sorted([
