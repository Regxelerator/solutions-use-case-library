...
from .agentic_flow_feedback import run_agentic_feedback 
...
@app.delete("/api/memo", status_code=204)
async def remove_entire_memo():
    delete_all()
...
@app.post("/api/memo/agentic_feedback")
async def integrate_feedback(payload: dict):

    feedback = payload.get("feedback", [])
    if not feedback:
        raise HTTPException(400, "feedback list is required")

    ci = payload.get("content_instructions", "").strip()
    si = payload.get("style_instructions", "").strip()
    user_instr = "\n\n".join(
        [ci and f"Content instructions:\n{ci}", si and f"Stylistic instructions:\n{si}"]
    ).strip()

    logger.info("Feedback integration started – %d item(s)", len(feedback))

    try:
        sections = await run_agentic_feedback(feedback, user_instr)
        logger.info("Feedback integration finished – draft updated")
        return {"sections": sections}

    except Exception:
        logger.exception("Feedback integration FAILED")
        raise HTTPException(500, "Feedback integration failed – see server.log for details")